...
class PdfPagesSplitterAction
{
    ...
    public function execute(string|SplFileInfo $input_file, string|SplFileInfo $output_dir)
    {
        ...
        $action->execute(dockerImageName: 'tools.duma.sh/scan-splitter/pdf-page-extractor',output: $output, return: $return);

        if (0 != $return) {
            $temporaryDirectory->delete();
            throw new PdfPageContentsExtractorException(
                command: $action->getCommand(dockerImageName: 'tools.duma.sh/scan-splitter/pdf-page-extractor'),
                code: $return,
                output: $output,
            );
        }
